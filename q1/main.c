 ...
 * 
 * 
 * 1.	Write an ioctl command to sort the first quantum of the 
 * first Sculldev in scull device(modify main.c)
 *
 */
	...
	switch(cmd) {
	  case SORT_FIRST_QUANTUM: 
	  	...
	  	for(int i=0; i<scull_quantum-1; i++)
	  	{
	  		...
	  		{
	  			if(*((char *)(d->data->data[0]+i)) > *((char *)(d->data->data[0]+j)))
	  			{
	  				temp=*((char *)(d->data->data[0]+i));
	  				*((char *)(d->data->data[0]+i))=*((char *)(d->data->data[0]+j));
