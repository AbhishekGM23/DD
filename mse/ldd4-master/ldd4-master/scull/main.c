 ...
 * 
 * 
 * Display and sort the second 50 characters (positions 50–100) 
 * of each of the 4 scull devices using the /proc filesystem.
 *
 */
...
static int scull_read_procmem(struct seq_file *s, void *v)
{
    ...
    for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
        ...
        /* extract second 50 characters (50–100) */
        for (j = 0; j < 50; j++) {
            temp[j] = *((char *)(qs->data[0] + 50 + j));
        }
        ...
        /* print */
        seq_printf(s, "\nDevice %d - Sorted second 50 chars:\n", i);

        for (j = 0; j < 50; j++) {
            seq_printf(s, "%c", temp[j]);
        }
