 ...
 * 
 * 
 * 1. copy the first and the third 10 character into a buffer. 
 *    use an ioctal to sort this buffer and copy the sorted contents 
*     to the first and the third 10 characters of the device
 *
 */
	...
	case SORT_FIRST_QUANTUM:
{
    ...
    char *q = (char *)d->data->data[0];

    /* Step 1: copy first 10 */
    for (i = 0; i < 10; i++)
        buf[i] = q[i];

    /* Step 2: copy third 10 */
    for (i = 0; i < 10; i++)
        buf[10 + i] = q[20 + i];

    /* Step 3: simple bubble sort (20 elements) */
    for (i = 0; i < 19; i++) {
        for (j = i + 1; j < 20; j++) {
            if (buf[i] > buf[j]) {
                temp = buf[i];
    ...
    printk(KERN_INFO "Sorted first and third 10 chars\n");
    break;
}
