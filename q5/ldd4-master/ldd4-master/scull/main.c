 ...
 * 
 * 5.	Display the last 50 characters of the  of each 
 * of the 4 scull devices using proc filesystem.
 *
 */
...
static int scull_read_procmem(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < scull_nr_devs && s->count <= limit; i++) {
		...
		int remaining;
		item = (long)d->size / itemsize;
		rest = (long)d->size % itemsize;
		...
		if (mutex_lock_interruptible(&d->mutex))
			return -ERESTARTSYS;
		seq_printf(s, "\nLast 50 characters of the Device %i\n", i);
		dptr=scull_follow(d, item);
		for(j=0; j<remaining; j++)
		{
			seq_printf(s, "%c", *((char *)(dptr->data[s_pos] + q_pos - remaining + j)));
		}
...
{
	proc_create_data("last50characters", 0 /* default mode */,
...
static void scull_remove_proc(void)
{
	...
	remove_proc_entry("last50characters", NULL /* parent dir */);
