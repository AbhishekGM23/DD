 ...
 * 
 * 
 * 2.	Write an ioctl command to sort the 
 * nth quantum of the first Sculldev in scull device(modify main.c)
 *
 */
...
long scull_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
	  case SORT_N_QUANTUM: 
	  	...
	  	if(!d->data->data)
	  		goto out;
	  	if(!d->data->data[nth_quantum])
	  		goto out;
	  	for(int i=0; i<scull_quantum-1; i++)
	  	{
	  		for(int j=i+1; j<scull_quantum; j++)
	  		{
	  			if(*((char *)(d->data->data[nth_quantum]+i)) > *((char *)(d->data->data[nth_quantum]+j)))
	  			{
	  				temp=*((char *)(d->data->data[nth_quantum]+i));
	  				*((char *)(d->data->data[nth_quantum]+i))=*((char *)(d->data->data[nth_quantum]+j));
	  				*((char *)(d->data->data[nth_quantum]+j))=temp;
	  			}
	  		}
	  	}
	  	break;
